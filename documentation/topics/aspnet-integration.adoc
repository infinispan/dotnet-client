...
Register {brandname} as the distributed cache provider in `Program.cs` or `Startup.cs`:
...
`CacheName`::
The name of the {brandname} cache to use.
Defaults to `"default"`.
...
The `DistributedCacheEntryOptions` are mapped to {brandname} expiration as follows:
...
Register both {brandname} and HybridCache in a single call:
...
{brandname} serves as the L2 distributed layer.
